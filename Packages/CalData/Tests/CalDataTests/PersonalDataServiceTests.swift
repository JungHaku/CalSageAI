...
struct PersonalDataServiceTests {
    ...
    private func makeStack() throws -> (
        ...
        SwiftDataJournalStore,
        RecordingRemoteMemory
    ) {
        ...
        let journal = SwiftDataJournalStore(modelContainer: container)
        let remote = RecordingRemoteMemory()
        return (
            PersonalDataService(
                checkIns: checkIns,
                profiles: profiles,
            ...
            checkIns, profiles, sessions, journal, remote
        )
    }
    ...
    private func seed(
        ...
        _ journal: SwiftDataJournalStore
    ) async throws {
        ...
        try await journal.save(
            JournalEntry(localDate: today, body: "Today felt steady.")
        )
    }

    @Test("an export gathers all four stores")
    func exportsEverything() async throws {
        let (service, checkIns, profiles, sessions, journal, _) = try makeStack()
        try await seed(checkIns, profiles, sessions, journal)
    ...
    func exportWindowCoversEverything() async throws {
        let (service, checkIns, _, _, _, _) = try makeStack()
    ...
    func exportIsValidJSON() async throws {
        ...
        #expect(decoded.journalEntries.count == archive.journalEntries.count)
    }
    ...
    func deleteErasesEverything() async throws {
        let (service, checkIns, profiles, sessions, journal, remote) = try makeStack()
        try await seed(checkIns, profiles, sessions, journal)
        ...
        #expect(try await journal.entries(from: LocalDate(iso: "2000-01-01")!, to: today).isEmpty)
        #expect(remote.forgetCount == 1, "server memories must be part of delete everything")
        #expect(remote.persistedGranted == false, "revoking consent is part of erasure")
    ...
    func usableAfterDelete() async throws {
        ...
        try await journal.save(JournalEntry(localDate: today, body: "Starting again."))
        #expect(try await journal.entries(from: today, to: today).count == 1)
    }
}

private final class RecordingRemoteMemory: RemoteMemoryControlling, @unchecked Sendable {
    private(set) var forgetCount = 0
    private(set) var persistedGranted: Bool?
    func persistConsent(granted: Bool) async throws { persistedGranted = granted }
    func forgetAll() async throws { forgetCount += 1 }
